package dev.koenv.libraryapi.server.routes
...
fun registerAllRoutes(root: Route) {
    val tree: List<RouteNode> = listOf(
        RouteGroup("/", listOf(
            dev.koenv.libraryapi.server.routes.root.RootRoutes,
        )),
        RouteGroup("/api", listOf(
            RouteGroup("/v1", listOf(
                // All authentication routes
                dev.koenv.libraryapi.server.routes.api.v1.auth.AuthRoutes,

                // User CRUD
                dev.koenv.libraryapi.server.routes.api.v1.users.UserRoutes,

                // Book CRUD
                dev.koenv.libraryapi.server.routes.api.v1.books.BookRoutes,

                // Loans CRUD
                dev.koenv.libraryapi.server.routes.api.v1.loans.LoanRoutes,
            ))
        ))
    )
